...
const BankersEvolutionChart = ({ bankerOrder }: BankersEvolutionChartProps) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (result.success && Array.isArray(result.data)) {
          ...
          const bankerNames = result.data.map(b => b.nome);
          ...
          if (result.data.length > 0 && result.data[0].evolution) {
            ...
            firstBanker.evolution.forEach((item: any, index: number) => {
              const datePoint: BankerCaptacaoData = { date: item.date };
              
              result.data.forEach(banker => {
                if (banker.evolution[index]) {
                  datePoint[banker.nome] = banker.evolution[index].value;
                }
              });
        ...
        >
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} style={{ color: entry.color }} className="text-xs font-semibold">
              {entry.name}: ${(entry.value as number).toLocaleString('en-US')}
            </p>
          ))}
        </motion.div>
      );
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-serif text-gold-gradient">P&L por Banker</h3>
          <p className="text-xs text-muted-foreground mt-1">Soma dos P&Ls dos clientes - Período de 01/12/2025 a 20/01/2026</p>
        </div>
              ...
              <div className="space-y-3 max-h-72 overflow-y-auto scrollbar-thin">
                {bankers.map((banker) => (
                  <label key={banker} className="flex items-center gap-3 cursor-pointer text-sm p-2 rounded hover:bg-prunus-green-light/40 transition-colors group">
                    ...
                    />
                    <span className="truncate group-hover:text-prunus-gold transition-colors">{banker}</span>
                  </label>
                ))}
              </div>
            </motion.div>
...
export default BankersEvolutionChart;
